...
function getDilPower() {
	var ret = Math.pow(3, player.dilation.rebuyables[3] * exDilationUpgradeStrength(3))
	if (player.masterystudies) {
...
function updateDimensions() {
    ...
    if (document.getElementById("stats").style.display == "block" && document.getElementById("statistics").style.display == "block") {
        ...
        var showCancer=player.spreadingCancer>0
...
function setAchieveTooltip() {
    ...
    sanic.setAttribute('ach-tooltip', "Have antimatter/sec exceed your current antimatter while above " + formatValue(player.options.notation, 1e63, 0, 0) + (player.mods.ac?"Reward: All dimensions are 80% stronger.":""));
...
function gameLoop(diff) {
    ...
    if (player.infMultBuyer && (!player.boughtDims || canBuyIPMult())) {
        var dif
        if (player.aarexModifications.newGameExpVersion) dif = Math.floor(player.infinityPoints.div(player.infMultCost).log(4)) + 1
